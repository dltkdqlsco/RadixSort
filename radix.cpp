...
void radix_sort()
{
    ...
    while (1)      // 최대 자릿수가 몇 자리인지 알아내기 위한 반복문
    {
        if (radix >= max_val) // max_val는 입력과 동시에 구해놓은 배열에서의 최댓값
            break;
        radix *= 10; // 10씩 곱해가면서 최대 자릿수 찾기
    }

    for (int i = 1; i < radix; i *= 10) // 1의 자리부터 10씩 곱하면서 최대자릿수까지 반복
    {
        ...
        {
            while (q[j].empty() == 0) // 해당 index번호의 queue가 비어있을 때 까지 반복
            {
...
int main()
{
    srand((unsigned)time(NULL)); // 현재 시간을 시드값으로 설정
    for (int i = 0; i < MAX;)
    {
        arr[i] = (rand() % 10000) + 1;
        if (flag[arr[i]] == false)
